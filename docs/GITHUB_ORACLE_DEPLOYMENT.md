...
monitoring port 8093, backup/resource guard and redacted diagnostic, use
...
For the initial bootstrap only, transfer the exact reviewed Bitcoin LIVE
...
- `/etc/bitcoin-live/.env`, mode `0600`, owned by `root:root`;
- dedicated `ghabtclive`, separate from the deployment user;
- `/var/lib/bitcoin-live/incoming`, mode `0700`, owned by `ghabtclive`;
- root-only staging and `/etc/bitcoin-live/approved-artifact.sha256`;
- root-owned `/usr/local/sbin/bitcoin-live-deploy`;
...
Docker group is root-equivalent; never add `ghabtclive` to it. The runner also
cannot read `/etc/bitcoin-live/.env`.

## 3. Create the private simulation configuration

Populate `/etc/bitcoin-live/.env` from `.env.example` through `sudo`; it must
...
```bash
sudo install -m 0600 -o root -g root .env.example /etc/bitcoin-live/.env
...
BINANCE_API_KEY=
BINANCE_API_SECRET=
SHARED_HOST_PATH=/var/lib/bitcoin-live/shared
```
...
by GitHub. Run installation and registration as `ghabtclive`, not as root or the
...
```text
/etc/bitcoin-live/approved-artifact.sha256
```
...
```bash
readlink -f /opt/bitcoin-live/current
sudo /usr/local/sbin/bitcoin-live-deploy verify
cat /var/lib/bitcoin-live/shared/runtime/deployment_status.json
cat /var/lib/bitcoin-live/shared/runtime/release_validation.json
cat /var/lib/bitcoin-live/shared/runtime/sidecar/sidecar_health.json
```
