...
Secrets live only in `/etc/bitcoin-live/.env` (`root:root`, mode 0600) and separate
root/bitcoinlivemon monitor env files. Config snapshots are mode 0600 under a mode-0700
...
`ghabtclive` outside Docker and privileged groups, separate it from the deployment
