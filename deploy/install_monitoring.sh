...
case "$MODE" in
  simulation|live) ;;
  *) fail 'Bitcoin LIVE monitoring mode must be simulation or live' ;;
esac
...
# /var/lib/bitcoin-live/config-snapshots, which oracle_setup.sh chowns to the
...
env_file_key_allowed() {
  case "$1" in
    ...
    MONEYFLOW_TRADE_LIMIT|MONITOR_ALLOWED_IPS|\
